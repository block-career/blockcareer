...
contract BlockCareer is MintableToken {
  string public constant name = "BLOCKCAREER";
 ...
 * Crowdsales have a start an d end block, where investors can make
...
contract BlockCareerCrowdsale is Crowdsale, Ownable {

  uint256 public platformSupply;
  uint256 public ITSupply;
  uint256 public administrativeSupply;
  uint256 public marketingSupply;
  uint256 public reserveSupply;

  uint256 public usedPlatformSupply;
  uint256 public usedITSupply;
  uint256 public usedAdministrativeSupply;
  ...
  function getUsedPlatformSupply() public returns(uint256) { return usedPlatformSupply; }
  function getUsedITSupply() public returns(uint256) { return usedITSupply; }
  function getUsedAdministrativeSupply() public returns(uint256) { return usedAdministrativeSupply; }
  function getUsedMarketingSupply() public returns(uint256) { return usedMarketingSupply; }
  function getUsedReserveSupply() public returns(uint256) { return usedReserveSupply; }

  BlockCareer blockcareerTokenContract;

  function BlockCareerCrowdsale(uint256 _startBlock, uint256 _endBlock, uint256 _rate, address _wallet) Crowdsale(_startBlock, _endBlock, _rate, _wallet) {
    platformSupply =        7500000 * 10**17;
    ITSupply =              5000000 * 10**17;
    administrativeSupply =  4500000 * 10**17;
    marketingSupply =       5000000 * 10**17;
    reserveSupply =         3000000 * 10**17;
  }

  // creates the token to be sold.
  // override this method to have crowdsale of a specific MintableToken token.
  function createTokenContract() internal returns (MintableToken) {
    return new BlockCareer();
  }
  ...
  function claimITTokens(address _to, uint256 _amount) payable onlyOwner {
    if (_amount > ITSupply - usedITSupply) revert();
    token.mint(_to, _amount);
    ITSupply += _amount;
  }
